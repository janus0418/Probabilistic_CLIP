...
class ClipLoss(nn.Module):
    ...
    def calculate_P_lessMemory_BEFORE_10_09_division_by_Sigma_Error(self,input,mu,sigma,device,b,d):
        # sigma --> DEFINED AS SD INSGTEQD OF VAR
        ...
        # mu = mu / torch.sqrt(sigma + 0.0000001)
        # input = input / torch.sqrt(sigma + 0.0000001)
        mu = mu / torch.sqrt(sigma + 1e-6)
        input = input / torch.sqrt(sigma + 1e-6)
        ...
        log_sigma = torch.log(sigma+ 1e-6) # MY EDIT: ADDED + 0.0000001 to sigma to avoid log(0) error
    ...
    def calculate_P_lessMemory_FULLRUN_10_10(self,input,mu,sigma,device,b,d):
        # b = sigma.shape[0]
        x = ((input.unsqueeze(1) - mu.unsqueeze(0)) ** 2) / (sigma.unsqueeze(0) + 1e-6) # [b, b, d]
        x = x.sum(-1) # [b, b]
        # log_sigma = torch.log(sigma+ 0.0000001)
        log_sigma = torch.log(sigma+ 1e-6)

        log_sigma = torch.sum(log_sigma, dim=1) # [batch x 1]
        return x + log_sigma
    ...
    def calculate_P_lessMemory(self,input,mu,sigma,device,b,d):
        ...
        # sig_recip = 1/(sigma + 0.0000001) # [batch_sig x dimension]
        # input_squared = torch.pow(input,2) @ (1/(sigma + 0.0000001)).T # [batch_input x batch_sig]
    
        # # mu_squared = # [batch_sig x dimension]
        # mu_squared = torch.sum(torch.pow(mu,2) / (sigma + 0.0000001), dim=1).reshape(1,mu.shape[0]) # 1 x batch_mu
        ...
        #mu_div_sig = torch.div(mu,sigma + 0.0000001) # [batch_sig x dimension]
        # middle = input @ torch.div(mu,sigma + 0.0000001).T # [batch_input x batch_sig]
        middle = input @ torch.div(mu,sigma + 1e-6).T # [batch_input x batch_sig]

        #Q = (-2)*middle + input_squared + mu_squared# [batch x batch]

        # log_sigma = torch.log(sigma+ 0.0000001)
        # log_sigma = torch.sum(torch.log(sigma+ 0.0000001), dim=1) # [batch x 1]
    ...
    def calculate_loss_less_JIAHAO_WORKING_VERSION(self,P,device):
        # find the main diagonal of res1
        diag = torch.diag(P).to(device)
    ...
    def calculate_loss_less(self,neg_log_unnormalized_P,device):
        """
        For neg_log_P, consider applying softmax along the rows.
        Note that P is of the following form:
            (input_1|given_1) ... (input_1|given_n)
            (input_2|given_1) ... (input_2|given_n)
            ...
            (input_n|given_1) ... (input_n|given_n)
        Hence, softmax over the rows will result in a model that requires the 
        iteration over the givens for the retrieval of the input. 
        For example, for text -> image retrieval, the model will fix the image
        and iterate over all the possible text conditions to find the best match
        """

        """
        # ORIGINAL CODE 
        log_unnormalized_P = - neg_log_unnormalized_P
        neg_log_P = - F.log_softmax(log_unnormalized_P, dim=1)
        # find the main diagonal of res1
        diag = torch.diag(neg_log_P).to(device)
        mean_of_right_matches = torch.mean(diag)
        """

        # New proposal 
        ## Do for both givens and inputs
        log_unnormalized_P = - neg_log_unnormalized_P
        neg_log_P_over_inputs = - F.log_softmax(log_unnormalized_P, dim=0)
        neg_log_P_over_givens = - F.log_softmax(log_unnormalized_P, dim=1)
        # find the main diagonal of res1
        diag_over_inputs = torch.diag(neg_log_P_over_inputs).to(device)
        diag_over_givens = torch.diag(neg_log_P_over_givens).to(device)

        mean_of_right_matches = torch.mean(diag_over_inputs+diag_over_givens)
        
        return mean_of_right_matches
    ...
    def forward(self, image_features, text_features, logit_scale, mu_img, sigma_img, mu_txt, sigma_txt):
        ...
        if self.world_size > 1:
            ...
            # print shape of all_image_features,all_text_features,all_mu_img,all_sigma_img,all_mu_txt,all_sigma_txt
            # print("shape of all_image_features: ", all_image_features.shape)
            # print("shape of all_text_features: ", all_text_features.shape)
            # print("shape of all_mu_img: ", all_mu_img.shape)
            # print("shape of all_sigma_img: ", all_sigma_img.shape)
            # print("shape of all_mu_txt: ", all_mu_txt.shape)
            # print("shape of all_sigma_txt: ", all_sigma_txt.shape)
            ...
            sigma_img_fin = all_sigma_img
            sigma_txt_fin = all_sigma_txt
            ...
            sigma_img_fin = sigma_img
            sigma_txt_fin = sigma_txt
        ...
        # EDIT 11/12/2023 Adding Regularization)

        if self.lambda_reg_l2 > 0:
            num_elements = torch.numel(sigma_img_fin) + torch.numel(sigma_txt_fin)
            norm = torch.norm(sigma_img_fin, p=1) + torch.norm(sigma_txt_fin, p=1)
            avg_norm = norm / (num_elements)

            # # option 1 -- use l2 norm; incentivize large values
            # LAMBDA_REG = 0.5/num_elements
            # l2_reg =  LAMBDA_REG * norm * self.lambda_reg_l2
            # total_loss = (loss_img + loss_txt) - l2_reg

            # option 2 -- use inverted norm; penalize small values
            # need to fix the outer torch.mean so that; replace + with ,
            l2_reg = torch.mean(torch.mean(torch.reciprocal(sigma_img_fin)) + torch.mean(torch.reciprocal(sigma_txt_fin)))
            total_loss = (loss_img + loss_txt) + self.lambda_reg_l2 * l2_reg
            
            # option 2
            # l2_reg = torch.meantorch.mean(1.0/sigma_img_fin) + torch.mean(1.0/sigma_txt_fin)
            # total_loss = (loss_img + loss_txt) + self.lambda_reg_l2 * l2_reg

        else: 
            num_elements = sigma_img_fin.shape[0]*sigma_img_fin.shape[1]
            norm = torch.norm(sigma_img_fin, p=1) + torch.norm(sigma_txt_fin, p=1)
            avg_norm = norm / (num_elements*2)
            l2_reg = 0
            total_loss = (loss_img + loss_txt)
        # print(f"##################  total_loss: {total_loss.item()}")
        return total_loss, avg_norm, l2_reg
