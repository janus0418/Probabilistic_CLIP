...
MLP_DEPTH = 2
class MLP(nn.Module):
    def __init__(self, is_Sigma = False, input_size=DIM, hidden_size=DIM*2, output_size=DIM, depth = MLP_DEPTH):
...
# DIM = 512
# MLP_DEPTH = 4  # Adjusted for a clearer bottleneck effect
...
#         # Implementing the bottleneck
#         for i in range(1, depth - 1):
#             # Calculate layer size - it decreases to the middle and then increases
#             layer_size = int(hidden_size / (2 ** abs(i - (depth // 2))))
#             layers.append(nn.Linear(hidden_size, layer_size))
#             layers.append(nn.ReLU())
#             hidden_size = layer_size  # Update hidden_size for next layer
